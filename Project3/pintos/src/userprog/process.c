...
/* For virtual memory */
#include "vm/page.h"                    // For using page functions
...
start_process (void *file_name_)
{
  ...
  /* For virtual memory */
  /* Initialize hast table of thread */
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      if(insert_vme(&thread_current()->vm, vm_e)!=true)
        printf("Fail to insert vm_entry \n");
...
setup_stack (void **esp) 
{
  ...
  /* Added variable */
  struct vm_entry *vm_e;
  void *v_addr = ((uint8_t *) PHYS_BASE) - PGSIZE;
  ...
  /* Initialize vm_entry field */
  vm_e->type = VM_ANON;
  vm_e->vaddr = pg_round_down(v_addr);
  vm_e->writable = true;
  vm_e->is_loaded = true;

  /* Insert vm_entry into hash_table */
  success = insert_vme(&thread_current()->vm, vm_e);
...
handle_mm_fault (struct vm_entry *vme)
{
  ...
  /* Check whether vme is already loaded or not */
  if(vme->is_loaded == true)
    return false;
  if(kaddr == NULL)
    return false;

  /* Load & Check the success in cases*/
  switch (vme->type)
  {
    /* Load the file */
    case VM_BIN:
    if(load_file(kaddr, vme)!=true)
    {
  ...
  /* Mapping the physical & virtual memory */ 
  if(install_page(vme->vaddr, kaddr, vme->writable) != true)
  {
    palloc_free_page(kaddr);
    return false;
  }
